...
// 可编辑多行文本域 v03
public class MultilineTextFieldWidget_v03 extends AbstractButtonWidget {
	private static final MinecraftClient CLIENT_INSTANCE = MinecraftClient.getInstance();
	...
	public MultilineTextFieldWidget_v03(TextRenderer textRenderer, int x, int y, int width, int height, Text text) {
		this(textRenderer, x, y, width, height, (TextFieldWidget) null, text);
	}

	public MultilineTextFieldWidget_v03(TextRenderer textRenderer, int x, int y, int width, int height, @Nullable TextFieldWidget copyFrom, Text text) {
		...
		if (copyFrom != null) {
			this.setString(copyFrom.getText());
		}
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		if (!this.isActive()) {
			...
			if (Screen.isSelectAll(keyCode)) {
				...
				CLIENT_INSTANCE.keyboard.setClipboard(this.getSelectedText());
				return true;
			} else if (Screen.isPaste(keyCode)) {
				if (this.editable) {
					this.write(CLIENT_INSTANCE.keyboard.getClipboard());
				}
	...
	public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if (this.isVisible()) {
			int j;
			if (this.drawsBackground()) { // 是否绘制背景
			...
			String string = this.textRenderer.trimToWidth(this.text.substring(this.firstCharacterIndex), this.getInnerWidth()); // 获取要显示的字符串
			boolean bl = k >= 0 && k <= string.length(); // A
